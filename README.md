...
- [B12. Change playbooks](#b12-change-playbooks)
...
```
 ...
 | _Archive (SQL    |  archive     | Docker: options_sqlserver        |
 |  Express)        |  pull 09:15  |   DB OptionsAdvisorDB            |
 | Task Scheduler   |              | stock_options_advisor :5001      |
...
| Hot DB | Docker **`OptionsAdvisorDB`** | Live operational data |
...
| `deploy/vm-restart.sh` / `update.sh` | Rebuild/restart app |
| `deploy/backup.sh` / `restore.sh` / `archive-export.sh` | VM backup helpers |
...
**EOD chain order** inside `morning_eod_catchup` (individual evening crons usually off):
...
Archive jobs run **inside** the app (pyodbc); `.bak` lands in `./backups` (SQL bind-mount). Friday clock times: [A5](#a5-clock-and-jobs).
...
ACK refuses if laptop never pulled, `LAST_HOT_BACKUP.json` missing/stale, or `*_Archive` grew after export.
...
`main.py` modes: default (scheduler + dashboard), `--dashboard-only`, `--scheduler-only`, `--ws-runner`, `--init-db`, `--check-db`, `--zerodha-login`.

```mermaid
flowchart LR
  subgraph compose [docker_compose]
    sql["options_sqlserver<br/>OptionsAdvisorDB"]
    app["stock_options_advisor<br/>main.py :5001"]
    ws["stock_ws_runner<br/>main.py --ws-runner"]
  end
  browser[Browser] --> app
  app --> sql
  ws --> sql
...
Env: `.env.docker` -> `OPT_DB_*`, `OPT_PROVIDERS`, Zerodha keys, `OPT_DASHBOARD_PORT`.  
Volumes: `sqlserver_data`, `./backups`, `./data`, `./logs`.  
...
stateDiagram-v2
  ...
  EXECUTED --> ACTIVE: options_trades
  ACTIVE --> CLOSED: exit / SL / milestone
  IGNORED --> SIM: simulation_update
```

Monitoring: WS ticks -> `live_risk_monitor` (MTM, milestones); fallback job `intraday_sl_fallback`; daily `exit_engine`.
...
Pricing provenance: `LIVE` | `EOD` | `MIXED`. Preserve `data/zerodha_session.json` across deploys ([A7](#a7-code-deploy)).
...
| Change | Touch |
|---|---|
| Confidence gate | `engine/confidence.py`, `config.py`, `tests/test_engine/`, README B7 |
| Strategy | `strategy_selector.py`, `leg_builder.py`, tests, README B7 |
...
| New module / boundary | README B1 / B6 |
