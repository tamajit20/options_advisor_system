...
**Amount needed Final/Peak:** suggestion **Amount needed** is Zerodha **Final** (full basket) for every strategy once a session quotes; Peak / Avail sit on the line underneath. Engine estimate only until Kite responds. Quotes once per suggestion+lots per browser session (tab reloads reuse cache); **Place orders in Zerodha** always fetches a live preview. Requires valid session (`zerodha_execution.enabled` + login) — place-orders toggle not required.
