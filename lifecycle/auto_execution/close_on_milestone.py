"""LOSS_MILESTONE_HIT / PROFIT_MILESTONE_HIT → flatten/book the open trade."""
...
def flatten_open_trade(db: SQLServerConnection, ctx: AutoExecContext) -> str:
    trd = TradeRepo(db)
...
class CloseOnProfitMilestone(AutoExecAction):
    name = "close_on_profit_milestone"
    notif_types: FrozenSet[str] = frozenset({"PROFIT_MILESTONE_HIT"})
    failure_notif_type = "PROFIT_MILESTONE_CLOSE_FAILED"
