...
def test_snap_bucket_truncates_to_15_minutes():
    dt = datetime(2026, 5, 22, 11, 47, 33)
    assert TradeMtmSnapshotRepo.snap_bucket(dt) == datetime(2026, 5, 22, 11, 45, 0)
...
def test_upsert_hourly_deletes_then_inserts():
    ...
    assert insert_args[2] == datetime(2026, 5, 22, 11, 45, 0)
...
def test_archive_trade_moves_rows():
    ...
    db.fetch_all.return_value = [{
        "trade_id": "TRD-20260506-001",
        "trade_name": "X",
        "snapshot_at": datetime(2026, 5, 22, 11, 45, 0),
        "snapshot_granularity": "15min",
        "mtm": -1000,
        "max_profit": 0,
        "max_loss": 5000,
        "dte": 4,
        "leg_ltps_json": "{}",
        "feed_source": "live",
        "created_at": datetime(2026, 5, 22, 11, 45, 0),
    }]
    cur = MagicMock()
    cur.rowcount = 1
