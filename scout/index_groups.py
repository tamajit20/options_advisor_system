...
from config import NIFTY_BANK_SYMBOLS

INDEX_GROUP_SPECS: Dict[str, dict] = {
...
def invalidate_index_tags_cache() -> None:
    global _SYMBOL_TAGS
    _SYMBOL_TAGS = {}
...
def _nifty50_rank() -> Dict[str, int]:
    return {s.upper(): i for i, s in enumerate(_nifty50_symbols())}


def index_groups_meta() -> Dict[str, dict]:
    """UI metadata for watchlist section headers (badge count is dynamic for Nifty 50)."""
    n50 = len(_nifty50_symbols())
    return {
        "nifty50": {
            **INDEX_GROUP_SPECS["nifty50"],
            "badge": str(n50),
        },
        "nifty_bank": dict(INDEX_GROUP_SPECS["nifty_bank"]),
    }
...
def _build_symbol_tags() -> Dict[str, List[str]]:
    ...
    for tags in out.values():
        tags.sort(key=lambda t: INDEX_GROUP_SPECS[t]["order"])
    return out
