...
_JOB_META: Dict[str, Dict[str, str]] = {
    ...
    "live_suggestion_engine": {"icon": "💡", "name": "Live Suggestion Engine",
                            "description": "Re-runs the suggestion engine against the live Zerodha chain hourly from 10:00–14:00 IST (Mon–Fri)."},
...
def _summarize_cron(cfg: Dict[str, Any]) -> str:
    ...
    if interval:
        return f"Every {int(interval)} min (skips when market closed) IST"
    schedules = cfg.get("schedules")
    if schedules:
        slots = [
            s for s in schedules
...
def _signal_kind_for_open_trade(
    ...
    *,
    ...
    if (
        ...
        return "exit"
    if risk in _MILESTONE_RISK_TYPES:
        return "milestone"
    if mtm is None:
        return None
    try:
